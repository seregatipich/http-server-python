...
def _request(method="GET", path="/", headers=None):
    return HttpRequest(method=method, path=path, headers=headers or {}, body=b"")
...
def test_echo_returns_path_suffix_as_text_plain(ctx):
    ...
    response = handler(_request(path="/echo/hello-world"), ctx)
...
def test_echo_handles_empty_suffix(ctx):
    ...
    handler = make_echo_handler(RecordingLogger())
    response = handler(_request(path="/echo/"), ctx)
...
def test_echo_logs_debug_event_with_content_length(ctx):
    ...
    handler(_request(path="/echo/abc"), ctx)
...
def test_echo_compresses_when_gzip_accepted(ctx):
    ...
    handler = make_echo_handler(RecordingLogger())
    response = handler(
        _request(path="/echo/payload", headers={"accept-encoding": "gzip"}), ctx
    )
...
def test_echo_applies_cors_headers_for_allowed_origin(ctx):
    ...
    response = handler(
        _request(path="/echo/x", headers={"origin": "https://app.example.com"}),
        ctx,
    )
...
def test_echo_close_connection_follows_request_preference(ctx):
    ...
    handler = make_echo_handler(RecordingLogger())
    closing = handler(_request(path="/echo/x", headers={"connection": "close"}), ctx)
    keep_alive = handler(_request(path="/echo/x"), ctx)
...
def test_user_agent_returns_header_value_as_text_plain(ctx):
    ...
    response = handler(
        _request(path="/user-agent", headers={"user-agent": "curl/8.4.0"}), ctx
    )
    assert response.status_line == "HTTP/1.1 200 OK"
...
def test_user_agent_returns_empty_when_header_absent(ctx):
    ...
    handler = make_user_agent_handler(RecordingLogger())
    response = handler(_request(path="/user-agent"), ctx)
    assert response.body == b""
...
def test_user_agent_logs_request_event(ctx):
    ...
    handler(_request(path="/user-agent", headers={"user-agent": "x"}), ctx)
...
def test_healthz_returns_200_when_not_draining(ctx):
    ...
    response = handler(_request(path="/healthz"), ctx)
    assert response.status_line == "HTTP/1.1 200 OK"
    assert response.body == b""
...
def test_healthz_logs_check_event_with_draining_flag(ctx):
    ...
    handler(_request(path="/healthz"), ctx)
